...
class AnalyzeGPA_Okadai:
    ...
    def get_gpa(self):
        ...
        for gpa_dict in self.gpa_list:
            year_completed_set.add(gpa_dict["year_completed"])

        year_completed_list = []
        for yc in year_completed_set:
            if not re.match(r".*またがり", yc):
        ...
        for gpa_dict in self.gpa_list:
            year_completed = gpa_dict["year_completed"]
            ...
            sum_gp_by_credits_dict[year_completed] +=\
                gpa_dict["gp"] * gpa_dict["credits"]
            sum_credits_dict[year_completed] += gpa_dict["credits"]
